...
func handleUploadVideo(c *gin.Context) {
	...
	err = DB.QueryRowContext(c, `
		...
		title, description, userID, videoContentTypeID, publishedContentStateID, uploadResult.SecureURL, uploadResult.SecureURL, // Usamos la misma URL como thumbnail por ahora
	).Scan(&videoID)
	if err != nil {
